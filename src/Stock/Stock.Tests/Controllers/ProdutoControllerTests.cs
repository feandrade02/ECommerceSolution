...
namespace Stock.Tests.Controllers
{
    public class ProdutoControllerTests
    {
        ...
        public async Task GetProductById_ExistingId_ReturnsOkObjectResult()
        {
            ...
            _mockRepo.Setup(repo => repo.GetProdutoByIdAsync(1)).ReturnsAsync(produto);
        ...
        {
            // Arrange
            _mockRepo.Setup(repo => repo.GetProdutoByIdAsync(It.IsAny<int>())).ReturnsAsync((Produto)null);

            // Act
            var result = await _controller.GetProdutoById(99);
        ...
        {
            // Arrange
            _mockRepo.Setup(repo => repo.GetProdutoByIdAsync(It.IsAny<int>())).ThrowsAsync(new Exception());
        ...
        public async Task AddProduct_ValidDto_ReturnsCreatedAtAction()
        {
            ...
            Assert.Equal(nameof(ProdutoController.GetProdutoById), createdAtActionResult.ActionName);
        ...
        public async Task UpdateProduct_ExistingIdAndValidDto_ReturnsOk()
        {
            ...
            _mockRepo.Setup(repo => repo.GetProdutoByIdAsync(produtoId)).ReturnsAsync(existingProduto);
            _mockRepo.Setup(repo => repo.SaveChangesAsync()).ReturnsAsync(true);
        ...
        public async Task UpdateProduct_NonExistingId_ReturnsNotFound()
        {
            ...
            var produtoId = 99;
            var produtoDto = new ProdutoDTO { Nome = "Atualizado", Preco = 15, QuantidadeEstoque = 8 };
            _mockRepo.Setup(repo => repo.GetProdutoByIdAsync(produtoId)).ReturnsAsync((Produto)null);
        ...
        {
            // Arrange & Act
            var resultPage = await _controller.GetAllProdutos(page: 0);
            var resultPageSize = await _controller.GetAllProdutos(pageSize: 0);
            var resultPrice = await _controller.GetAllProdutos(minPrice: 100, maxPrice: 50);
            var resultStock = await _controller.GetAllProdutos(minStock: 100, maxStock: 50);
        ...
        public async Task GetAllProducts_ReturnsOkWithProducts()
        {
            ...
            _mockRepo.Setup(repo => repo.GetAllProdutosAsync(
                It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>(), It.IsAny<string>(),
                It.IsAny<bool>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>()))
